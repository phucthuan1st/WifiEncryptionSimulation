...
#include <stdint.h>
#include <string.h>
#include <atlstr.h> // for CString class
...
LRESULT CClientDlg::SockMsg(WPARAM wParam, LPARAM lParam)
{
	...
	case FD_READ:
	{
		...
		case 3://nhận message của các client khác được gửi lại từ sever.
		{
			...
			CString m_newmsgRecv;
			if (Mode == "AES") {

				uint8_t iv[AES_BLOCK_SIZE] = { 0 };

				CStringA strA(strResult[2]); // Chuyển đổi CString sang CStringA (dạng ký tự đơn byte)
				const size_t m_msgRecv_len = strA.GetLength(); // Lấy độ dài của chuỗi CStringA
				uint8_t* message = reinterpret_cast<uint8_t*>(strA.GetBuffer(m_msgRecv_len)); // Chuyển đổi CStringA sang uint8_t*
				...
				m_newmsgRecv.Format(_T("%s"), (char*)message);
				m_msgString += m_newmsgRecv + _T("\r\n");
			}
			if (Mode == "WEP") {
				...
				m_newmsgRecv.Format(_T("%s"), ciphertext);
				m_msgString += m_newmsgRecv + _T("\r\n");
			}
			UpdateData(FALSE);
		}break;
...
void CClientDlg::OnBnClickedSend()
{
	...
	if (Mode == "AES") {
		...
		const size_t m_msgSend_len = strA.GetLength(); // Lấy độ dài của chuỗi CStringA
		uint8_t* message = reinterpret_cast<uint8_t*>(strA.GetBuffer(m_msgSend_len)); // Chuyển đổi CStringA sang uint8_t*
		strA.ReleaseBuffer(); // Giải phóng bộ đệm của CStringA
