...
uint8_t* convertCstringtoUint8(CString str) {
	std::vector<uint8_t> arr(str.GetLength() / 2); // tạo một vector để lưu trữ mảng uint8_t kết quả

	for (int i = 0, j = 0; i < str.GetLength(); i += 2, j++) {
		CString byte_str = str.Mid(i, 2); // tách mỗi cặp ký tự trong chuỗi và tạo một CString mới
		arr[j] = stoi((LPCTSTR)byte_str, nullptr, 16); // chuyển đổi CString mới sang uint8_t dưới cơ số 16
	}
	uint8_t* result = new uint8_t[arr.size()]; // cấp phát một vùng nhớ động cho mảng uint8_t kết quả
	std::copy(arr.begin(), arr.end(), result); // sao chép dữ liệu từ vector vào mảng động

	return result; // trả về con trỏ đến mảng uint8_t động
}

CString convertUint8toCstring(const uint8_t* arr, size_t size) {
	CString result;
	for (size_t i = 0; i < size; i++) {
		CString byte_str;
		byte_str.Format(_T("%02X"), arr[i]); // chuyển đổi từng byte của mảng uint8_t thành một chuỗi ký tự theo cơ số 16, có độ dài là 2 ký tự
		result += byte_str; // ghép chuỗi kết quả
	}
	return result;
}
	...
	case FD_READ:
	{
		...
		case 3://nhận message của các client khác được gửi lại từ sever.
		{
			...
			if (Mode == "AES") {
				...
				uint8_t* message = convertCstringtoUint8(strResult[2]);
				uint8_t* key = convertCstringtoUint8(m_Pass);
				CBCCtrDec(iv, message, key);
				m_msgString += convertUint8toCstring(message, 16) + _T("\r\n");
			}
...
void CClientDlg::OnBnClickedSend()
{
	...
	if (Mode == "AES") {
		...
		uint8_t iv[AES_BLOCK_SIZE] = { 0 };
		uint8_t* message = convertCstringtoUint8(m_msgSend);
		uint8_t* key = convertCstringtoUint8(m_Pass);
		CBCCtrEnc(iv, message, key);
		Command += convertUint8toCstring(message,16) + _T("\r\n");
	}
	mSend(Command);
	m_msgSend = _T("");
	UpdateData(FALSE);
