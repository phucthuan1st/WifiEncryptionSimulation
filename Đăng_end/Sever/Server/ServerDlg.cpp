...
CServerDlg::CServerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CServerDlg::IDD, pParent), m_msgString(_T(""))
	//, ManagedClient(_T(""))
	,
	OnlineClient(_T("")), v_IP(_T("")), v_Port(_T(""))
{
...
void CServerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_PASS, v_Pass);
	DDX_Control(pDX, IDC_Algorithm, v_Algorithm);
}
...
BEGIN_MESSAGE_MAP(CServerDlg, CDialog)
	...
	ON_EN_CHANGE(IDC_IP, &CServerDlg::OnEnChangeIp)
	ON_EN_CHANGE(IDC_PORT, &CServerDlg::OnEnChangePort)
	ON_EN_CHANGE(IDC_PASS, &CServerDlg::OnEnChangePass)
	ON_CBN_EDITCHANGE(IDC_Algorithm, &CServerDlg::OnCbnEditchangeAlgorithm)
END_MESSAGE_MAP()
...
void CServerDlg::Split(CString src, CString des[8])
{
	...
	p6 = src.Find(_T("\r\n"), p2 + 1);
	des[5] = src.Mid(p5 + 2, p6 - (p5 + 2));

	p7 = src.Find(_T("\r\n"), p2 + 1);
	des[6] = src.Mid(p6 + 2, p7 - (p6 + 2));
}
...
void CServerDlg::OnBnClickedListen()
{
	...
	loadData();
	UpdateData(TRUE);
	// get pass and get mode
	v_Pass.GetWindowTextW(str_pass);
	v_Algorithm.GetWindowTextW(Mode);
	if (str_pass == "")
	{
	...
	serverAdd.sin_port = htons(_ttoi(v_Port));
...
LRESULT CServerDlg::SockMsg(WPARAM wParam, LPARAM lParam)
{
	...
	case FD_ACCEPT:
	{
		...
		GetDlgItem(IDC_LISTEN)->EnableWindow(FALSE);
		// send Mode algorithm
		mSend(pSock[number_Socket].sockClient, Mode);
		//MessageBox(Mode);
		break;
	}
	case FD_READ:
	{
		...
		case 1://Login
		{
			...
			//	}
			//}
			if (str_pass != mes2/*checkLogin(mes2) != 1*/ /*check xem user có nhập đúng pass gán cứng của wifi*/)
			{
				t = 1;
			}
			if (t == 0)
...
int CServerDlg::checkLogin(char* Pass)
{
	fstream file;
...
void CServerDlg::OnEnChangeIp()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	// with the ENM_CHANGE flag ORed OnEnChangePort2into the mask.

	// TODO:  Add your control notification handler code here
}

void CServerDlg::OnEnChangePort()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CServerDlg::OnEnChangePass()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CServerDlg::OnCbnEditchangeAlgorithm()
{
	// TODO: Add your control notification handler code here
}
